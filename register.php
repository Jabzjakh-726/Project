...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nameErr) && empty($passErr) && empty($emailErr) && empty($birthErr) && empty($phnoErr) && empty($genErr) && empty($repassErr) && empty($fileuploadErr) && empty($addErr) && empty($fileuploadErr) && empty($cErr)) {
        ...
        $username = "root";
        $password = "";
        $conn = mysqli_connect($servername, $username, $password,'db3');
        $result=mysqli_query($conn,"insert into tenth(name,email,pass,repass,exam,phno,gender,birthdate,address) values('$uname','$email','$pass','$repass','$exam','$phno','$gen','$birth','$add');") or $val='email already exists';
